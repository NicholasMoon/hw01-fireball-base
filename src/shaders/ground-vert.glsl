...
void main()
{
    ...
    float displacement_factor = 2.0f;

    

    float fbm_val = fbm3D(vec3(vs_Pos.x,0, vs_Pos.z), u_NoiseAmp[1], u_NoiseFreq[1] , u_NoisePersistence[1], int(u_NoiseOctaves[1]));

    float worley_val = 1.0f - worley3D(vec3(vs_Pos.x,0, vs_Pos.z), u_Freq[2]);


    vec4 newPos = vec4(0, (worley_val + fbm_val) * displacement_factor,0,0) + vs_Pos;
