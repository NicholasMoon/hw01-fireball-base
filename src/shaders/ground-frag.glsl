...
vec3 getTerrainColor(vec3 p) {
    ...
    float fbm_val = fbm3D(p, u_NoiseAmp[1], u_NoiseFreq[1], u_NoisePersistence[1], int(u_NoiseOctaves[1]));

    vec3 a = vec3(0.5,0.5,0.5);
    vec3 b = vec3(0.8425,0.35,0.155);
    vec3 c = vec3(0.312,0.74,0.4147);
    vec3 d = vec3(0.527,0.366,0.66);
    
    color += getCosinePaletteColor(a,b,c,d, fbm_val);

    float worley_val = worley3D(fs_Pos.xyz, u_Freq[2]);

    a = vec3(0.5,0.5,0.5);
    b = vec3(0.2425,0.214235,0.5155);
    c = vec3(0.312,0.574,0.4147);
    d = vec3(0.7527,0.4366,0.266);
    
    color += getCosinePaletteColor(a,b,c,d, worley_val);
...
void main()
{
    ...
    float myTime = float(u_Time) / 100.0f;

    vec3 color = vec3(0,0,0);

    color += getTerrainColor(fs_Pos.xyz);
    ...
    out_Col = vec4(color * inverse_square_falloff * 4.0 + (u_LightCol.xyz * inverse_square_falloff * lightIntensity), 1.0);
